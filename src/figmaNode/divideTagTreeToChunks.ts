import { APPROX_TOKEN_COUNT_PER_CHARACTER, MAX_TOKEN_COUNT_FOR_CODE } from "../constants";
...
export const divideTagTreeToChunks = (tag: Tag): Tag[] => {
	...
	const traverse = (node: Tag, childIndex: number, parentNodeRef: Tag | null) => {
	...
	nodes.forEach(({ node, childIndex, parentNodeRef }) => {
		...
		if (tokenCount < MAX_TOKEN_COUNT_FOR_CODE && "id" in node && parentNodeRef && "children" in parentNodeRef) {
