...
const TreeNode = ({ originalNodeTree, selectedNodes, onSelectNode, expandAll, searchQuery, isRootChild }) => {
	...
	const shouldShowNode = originalNodeTree.name.toLowerCase().includes(searchQuery.toLowerCase());
	...
	return (
		<div className={styles["tree-node"]}>
			<div className={`${styles["node-content"]} ${isSelected ? styles.selected : ""}`} onClick={handleSelectNode}>
				{originalNodeTree.children && originalNodeTree.children.length > 0 && (
					<span className={`${styles["toggle-icon"]} ${isOpen ? "open" : styles.closed}`} onClick={handleNodeClick}>
						▸
					</span>
				)}
...
export const LayerTree: React.FC<{ originalNodeTree: OriginalNodeTree }> = ({ originalNodeTree }) => {
	...
	const findNodeById = (id: string, tree: OriginalNodeTree): OriginalNodeTree | null => {
		if (tree.id === id) {
			return tree;
		}
	...
	const selectedNodeNames = selectedNodes.map((id) => findNodeById(id, originalNodeTree)?.name || "").join(", ");
